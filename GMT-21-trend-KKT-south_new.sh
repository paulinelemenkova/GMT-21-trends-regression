...
gmt psxy -R-200/200/-10000/1000 -JX15c/4c \
    ...
    --MAP_GRID_PEN_SECONDARY=thinnest,dimgray \
    -BWSne \
    -Glightgray env1.txt -Y5.5c -O -K >> $ps
gmt psxy -R -J -W0.05p -Ey+p0.2p stack1.txt -O -K >> $ps
gmt psxy -R -J -W1.0p,red stack1.txt \
    -O -K >> $ps

# Step-9. Add test annotations
echo "-150 -2000 Greater Kuril Chain" | gmt pstext -R -J -Gwhite -F+jTC+f11p -O -K >> $ps
echo "50 1500 Median stacked profile with error bars" | gmt pstext -R -J -Gwhite -F+jTC+f11p,red -O -K >> $ps
echo "0 -100 Kuril-Kamchatka Trench" | gmt pstext -R -J -Gwhite -F+jTC+f11p -O -K >> $ps
echo "150 -6500 Pacific Plate" | gmt pstext -R -J -Gwhite -F+jTC+f11p -O -K >> $ps
...
gmt psconvert KKTtrends_S14j.ps -A2.0c -E720 -Tj -P -Z
# Step-13. Clean up
rm -f model.txt
