#!/bin/bash
# Purpose: Regression trend1d mixed models, Japan Trench (NORTHERN segment)
...
gmt trend1d -Fxm stackJTn.txt -Np1 > model.txt
gmt psxy -R-200/200/-9000/3000 -JX15c/4c -P -Bpxag100f10 -Bsxg50 -Bya2000f+u"m" -Bsyg2000 \
    ...
    -BWSne+gazure1 -Sc0.05c -Gred stackJTn.txt -X5.0c -K > $ps
...
gmt trend1d -Fxm stackJTn.txt -Np2 > model.txt
gmt psxy -R -J -Bpxag100f10 -Bsxg50 -Bpyaf+u"m" -Bsyg2000 \
    --MAP_GRID_PEN_PRIMARY=thinnest,dimgray \
    --MAP_GRID_PEN_SECONDARY=thinnest,dimgray \
    -BWSne+gazure1 -Sc0.05c -Gred stackJTn.txt -Y5.0c -O -K >> $ps
...
gmt trend1d -Fxmr stackJTn.txt -Np2,f1+l1 > model.txt
gmt psxy -R -J -Bpxag100f10 -Bsxg50 -Bpyaf+u"m" -Bsyg2000 \
    --MAP_GRID_PEN_PRIMARY=thinnest,dimgray \
    --MAP_GRID_PEN_SECONDARY=thinnest,dimgray \
    -BWSne+gazure1 -Sc0.05c -Gred stackJTn.txt -Y5.0c -O -K >> $ps
...
# Step-8. Plot graph
gmt psxy -R-200/200/-9000/2000 -JX15c/4c \
    -Bpxag100f10+l"Distance from trench (km)" -Bsxg50 -Bpya2000g2000f+l"Depth (m)" -Bsyg2000 \
    --MAP_GRID_PEN_PRIMARY=thinnest,dimgray \
    --MAP_GRID_PEN_SECONDARY=thinnest,dimgray \
-BWSne+t"Modelled trends of the geomorphic slope gradients in profiles curves" \
    -Glightgray -W0.5p envJTn.txt -Y5.5c -O -K >> $ps
gmt psxy -R -J -W0.05p -Ey+p0.2p stackJTn.txt -O -K >> $ps
gmt psxy -R -J -W1.0p,red stackJTn.txt \
    -O -K >> $ps
...
echo "-70 500 NORTHERN segment: median stacked profile with error bars" | gmt pstext -R -J -Gwhite -F+jBL+f10p,red -O -K >> $ps
echo "-190 300 Honshu Island" | gmt pstext -R -J -Gwhite -F+jBL+f10p,darkbrown -O -K >> $ps
echo "0 -1000 Japan Trench" | gmt pstext -R -J -Gwhite -F+jBL+f10p,navyblue -O -K >> $ps
...
echo "170 -1500 A" | gmt pstext -R -J -F+jBL+f18p,black -Gfloralwhite -W0.5p -O -K >> $ps
# Arrow
...
gmt psconvert JTntrends.ps -A1.0c -E720 -Tj -P -Z
# Step-13. Clean up
rm -f model.txt
