#!/bin/bash
# Purpose: Regression trend1d mixed models, Izu-Bonin Trench (NORTHERN segment)
...
gmt trend1d -Fxm stackIBTn.txt -Np1 > model.txt
gmt psxy -R-200/200/-11000/3000 -JX15c/4c -P -Bpxag100f10 -Bsxg50 -Bya2000f+u"m" -Bsyg2000 \
    ...
    -BWSne+gazure1 -Sc0.05c -Gred stackIBTn.txt -X5.0c -K > $ps
...
gmt trend1d -Fxm stackIBTn.txt -Np2 > model.txt
gmt psxy -R -J -Bpxag100f10 -Bsxg50 -Bpyaf+u"m" -Bsyg2000 \
    --MAP_GRID_PEN_PRIMARY=thinnest,dimgray \
    --MAP_GRID_PEN_SECONDARY=thinnest,dimgray \
    -BWSne+gazure1 -Sc0.05c -Gred stackIBTn.txt -Y5.0c -O -K >> $ps
...
gmt trend1d -Fxmr stackIBTn.txt -Np2,f1+l1 > model.txt
gmt psxy -R -J -Bpxag100f10 -Bsxg50 -Bpyaf+u"m" -Bsyg2000 \
    --MAP_GRID_PEN_PRIMARY=thinnest,dimgray \
    --MAP_GRID_PEN_SECONDARY=thinnest,dimgray \
    -BWSne+gazure1 -Sc0.05c -Gred stackIBTn.txt -Y5.0c -O -K >> $ps
...
-BWSne+t"Izu-Bonin Trench: modelled trends of the slope gradients in the profiles curves" \
    -Glightgray -W0.5p envIBTn.txt -Y5.5c -O -K >> $ps
gmt psxy -R -J -W0.05p -Ey+p0.2p stackIBTn.txt -O -K >> $ps
gmt psxy -R -J -W1.0p,red stackIBTn.txt \
    -O -K >> $ps
...
gmt psconvert IBTntrends.ps -A1.0c -E720 -Tj -P -Z
# Step-13. Clean up
rm -f model.txt
