#!/bin/bash
# Purpose: Regression trend1d mixed models, San Cristobal Trench
...
gmt trend1d -Fxm stackSCT.txt -Np1 > model.txt
gmt psxy -R-200/200/-9000/3000 -JX15c/4c -P -Bpxag100f10 -Bsxg50 -Bya2000f+u"m" -Bsyg2000 \
    ...
    -BWSne+gazure1 -Sc0.05c -Gred stackSCT.txt -X5.0c -K > $ps
...
gmt trend1d -Fxm stackSCT.txt -Np2 > model.txt
gmt psxy -R -J -Bpxag100f10 -Bsxg50 -Bpyaf+u"m" -Bsyg2000 \
    --MAP_GRID_PEN_PRIMARY=thinnest,dimgray \
    --MAP_GRID_PEN_SECONDARY=thinnest,dimgray \
    -BWSne+gazure1 -Sc0.05c -Gred stackSCT.txt -Y5.0c -O -K >> $ps
...
gmt trend1d -Fxmr stackSCT.txt -Np2,f1+l1 > model.txt
gmt psxy -R -J -Bpxag100f10 -Bsxg50 -Bpyaf+u"m" -Bsyg2000 \
    --MAP_GRID_PEN_PRIMARY=thinnest,dimgray \
    --MAP_GRID_PEN_SECONDARY=thinnest,dimgray \
    -BWSne+gazure1 -Sc0.05c -Gred stackSCT.txt -Y5.0c -O -K >> $ps
...
gmt psxy -R-200/200/-11000/3000 -JX15c/4c \
    ...
    --MAP_GRID_PEN_PRIMARY=thinnest,dimgray \
    --MAP_GRID_PEN_SECONDARY=thinnest,dimgray \
    -BWSne+t"Graph of the modelled trend curves of the San Cristobal Trench geomorphology" \
    -Glightgray -W0.5p envSCT.txt -Y5.5c -O -K >> $ps
gmt psxy -R -J -W0.05p -Ey+p0.2p stackSCT.txt -O -K >> $ps
gmt psxy -R -J -W1.0p,red stackSCT.txt \
    -O -K >> $ps

# Step-9. Add test annotations
echo "-145 -2500 Solomon Islands" | gmt pstext -R -J -Gwhite -F+jBL+f10p,brown -O -K >> $ps
...
echo "-95 -3500 Continental slope" | gmt pstext -R -J -F+jBL+f11p,darkbrown+a-40 -Gwhite -O -K >> $ps
...
gmt psconvert SCTtrends.ps -A1.0c -E720 -Tj -P -Z
# Step-13. Clean up
rm -f model.txt
