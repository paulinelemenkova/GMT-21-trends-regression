...
gmt trend1d -Fxm stackYT.txt -Np1 > model.txt
gmt psxy -R-200/200/-9000/3000 -JX15c/4c -P -Bpxag100f10 -Bsxg50 -Bya2000f+u"m" -Bsyg2000 \
    ...
    -BWSne+gazure1 -Sc0.05c -Gred stackYT.txt -X5.0c -K > $ps
...
gmt trend1d -Fxm stackYT.txt -Np2 > model.txt
gmt psxy -R -J -Bpxag100f10 -Bsxg50 -Bpyaf+u"m" -Bsyg2000 \
    --MAP_GRID_PEN_PRIMARY=thinnest,dimgray \
    --MAP_GRID_PEN_SECONDARY=thinnest,dimgray \
    -BWSne+gazure1 -Sc0.05c -Gred stackYT.txt -Y5.0c -O -K >> $ps
...
gmt trend1d -Fxmr stackYT.txt -Np2,f1+l1 > model.txt
gmt psxy -R -J -Bpxag100f10 -Bsxg50 -Bpyaf+u"m" -Bsyg2000 \
    --MAP_GRID_PEN_PRIMARY=thinnest,dimgray \
    --MAP_GRID_PEN_SECONDARY=thinnest,dimgray \
    -BWSne+gazure1 -Sc0.05c -Gred stackYT.txt -Y5.0c -O -K >> $ps
...
gmt psxy -R-200/200/-9000/2000 -JX15c/4c \
    ...
    --MAP_GRID_PEN_PRIMARY=thinnest,dimgray \
    --MAP_GRID_PEN_SECONDARY=thinnest,dimgray \
    -BWSne+t"Graph of the modelled trend curves of the Yap Trench geomorphology" \
    -Glightgray -W0.5p envYT.txt -Y5.5c -O -K >> $ps
gmt psxy -R -J -W0.05p -Ey+p0.2p stackYT.txt -O -K >> $ps
gmt psxy -R -J -W1.0p,red stackYT.txt \
    -O -K >> $ps
...
echo "-5 1000 Yap Trench" | gmt pstext -R -J -Gwhite -F+jBL+f10p,navyblue -O -K >> $ps
...
gmt psconvert YTtrends.ps -A1.0c -E720 -Tj -P -Z
# Step-13. Clean up
rm -f model.txt
