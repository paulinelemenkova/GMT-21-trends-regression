...
echo "180 -1500 I" | gmt pstext -R -J -F+jBL+f18p,black -Gfloralwhite -W0.5p -O -K >> $ps

# Step-6. Basic LS line y = a + bx + cx^2 + spatial change
gmt trend1d -Fxmr stackT.txt -Np2,f1+l1 > model.txt
gmt psxy -R -J -Bpxag100f10 -Bsxg50 -Bpyaf+u"m" -Bsyg2000 \
    --MAP_GRID_PEN_PRIMARY=thinnest,dimgray \
    --MAP_GRID_PEN_SECONDARY=thinnest,dimgray \
    -BWSne+gazure1 -Sc0.05c -Gred stackT.txt -Y5.0c -O -K >> $ps
...
echo "180 -1500 H" | gmt pstext -R -J -F+jBL+f18p,black -Gfloralwhite -W0.5p -O -K >> $ps
...
echo "@~e@~(t) = y(t) - m@-5@-(t) (plot of the residuals)" | gmt pstext -R -J \
    -F+f11p+cBL -Dj0.1i -Glightyellow -O -K >> $ps
echo "180 1000 G" | gmt pstext -R -J -F+jBL+f18p,black -Gfloralwhite -W0.5p -O -K >> $ps
...
gmt psxy -R-200/200/-11000/1000 -JX15c/4c \
    ...
    --MAP_GRID_PEN_SECONDARY=thinnest,dimgray \
    -BWSne \
    -Glightgray -W0.5p envT.txt -Y5.5c -O -K >> $ps
gmt psxy -R -J -W0.05p -Ey+p0.2p stackT.txt -O -K >> $ps
gmt psxy -R -J -W1.0p,red stackT.txt \
    -O -K >> $ps

# Step-9. Add test annotations
echo "-150 -3000 Tonga Ridge" | gmt pstext -R -J -Gwhite -F+jTC+f11p -O -K >> $ps
echo "50 1500 Median stacked profile with error bars" | gmt pstext -R -J -Gwhite -F+jTC+f11p,red -O -K >> $ps
echo "0 -100 Tonga Trench" | gmt pstext -R -J -Gwhite -F+jTC+f11p -O -K >> $ps
...
echo "-120 -6000 Oceanward Forearc" | gmt pstext -R -J -F+jBL+f11p,darkbrown+a-27 -Gwhite -O -K >> $ps
echo "180 -1500 F" | gmt pstext -R -J -F+jBL+f18p,black -Gfloralwhite -W0.5p -O -K >> $ps
# Arrow
...
gmt psconvert KTTtrends_Tonga.ps -A2.0c -E720 -Tj -P -Z
# Step-13. Clean up
rm -f model.txt
