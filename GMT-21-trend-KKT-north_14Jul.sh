...
gmt psxy -R-200/200/-10000/1000 -JX15c/4c \
    ...
    --MAP_GRID_PEN_SECONDARY=thinnest,dimgray \
    -BWSne \
    -Glightgray env2.txt -Y5.5c -O -K >> $ps
gmt psxy -R -J -W0.1p -Ey+p0.2p stack2.txt -O -K >> $ps
...
echo "0 -1000 Kuril-Kamchatka Trench" | gmt pstext -R -J -Gwhite -F+jTC+f11p -O -K >> $ps
echo "-150 -5000 Greater Kuril Chain" | gmt pstext -R -J -Gwhite -F+jTC+f11p -O -K >> $ps
echo "180 -1500 F" | gmt pstext -R -J -F+jBR+f18p,black -Gfloralwhite -W0.5p -O -K >> $ps
# Arrow
...
gmt psconvert KKTtrends_N14j.ps -A2.0c -E720 -Tj -P -Z
# Step-13. Clean up
rm -f model.txt
